# מדדים להערכת סיכון { #risk-measures }
---
...
## מדדי ביצועים לסיכון מוחלט

מדדים אלו מאפשרים למשקיעים להעריך את מידת הסיכון בהשקעה ללא קשר לתשואה שהתקבלה.
מדדים אלה עונים על השאלה - מהי מידת הסיכון שנטלתי על עצמי?"

### סטיית תקן ותנודתיות { #std }
...
### בטא- מדידת הסיכון היחסי לשוק { #beta }
...
מדד שארפ (Sharpe Ratio) הוא אחד המדדים הנפוצים ביותר להערכת ביצועים מתוקנני-סיכון. הוא מודד את התשואה העודפת (תשואת התיק פחות תשואת נכס חסר סיכון) ביחס לסטיית התקן (הסיכון הכולל) של התיק.
...
**מדד סורטינו** (Sortino Ratio) הוא שיפור של מדד שארפ, המתמקד אך ורק ב**סיכון בצד השלילי (Downside Risk)**. במקום להשתמש בסטיית התקן הכוללת (שכוללת גם תנודתיות חיובית), מדד סורטינו משתמש ב**סטיית תקן יורדת (Downside Deviation)** - מדידה של פיזור התשואות רק מתחת לרף מסוים (לרוב תשואה חסרת סיכון או אפס).
